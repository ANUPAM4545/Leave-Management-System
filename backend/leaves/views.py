...
from .models import LeaveRequest, LeaveType, LeaveAuditLog, Holiday
from .serializers import LeaveRequestSerializer, LeaveTypeSerializer, HolidaySerializer
...
class HolidayListView(generics.ListAPIView):
    ...
    Includes a sync mechanism to fetch realtime data if the database is empty.
    """
    serializer_class = HolidaySerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        # Trigger sync if no holidays found
        if not Holiday.objects.exists():
            self.sync_holidays()
        
        return Holiday.objects.filter(date__gte=timezone.now().date()).order_by('date')[:5]

    def sync_holidays(self):
        """
        Syncs holidays from Nager.Date API for the current year.
        Defaults to India (IN).
        """
        year = timezone.now().year
        country_code = "IN" # Default based on system context
            ...
            with urllib.request.urlopen(req, context=context) as response:
                ...
                for item in data:
                    Holiday.objects.get_or_create(
                        date=item['date'],
                        defaults={'name': item['localName'], 'is_public': True}
                    )
        except Exception as e:
            print(f"Failed to sync holidays: {e}")
